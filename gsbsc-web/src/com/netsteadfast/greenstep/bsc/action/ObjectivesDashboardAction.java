...
public class ObjectivesDashboardAction extends BaseJsonAction implements IBaseAdditionalSupportAction {
	...
	private void checkFields() throws ControllerException, Exception {
		...
		.add("visionOid", SelectItemFieldCheckUtils.class, this.getText("MESSAGE.BSC_PROG003D0005Q_visionOid") )
		.add("frequency", SelectItemFieldCheckUtils.class, this.getText("MESSAGE.BSC_PROG003D0005Q_frequency") )
		...
		if ( BscMeasureDataFrequency.FREQUENCY_DAY.equals(frequency) 
			...
			if ( StringUtils.isBlank( startDate ) || StringUtils.isBlank( endDate ) ) {
				super.throwMessage("startDate|endDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg1"));			
			}
			if ( !StringUtils.isBlank( startDate ) || !StringUtils.isBlank( endDate ) ) {
				if ( !SimpleUtils.isDate( startDate ) ) {
					super.throwMessage("startDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg3"));
				}
				if ( !SimpleUtils.isDate( endDate ) ) {
					super.throwMessage("endDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg4"));		
				}
				if ( Integer.parseInt( endDate.replaceAll("/", "").replaceAll("-", "") )
						< Integer.parseInt( startDate.replaceAll("/", "").replaceAll("-", "") ) ) {
					super.throwMessage("startDate|endDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg5"));	
				}			
			}			
		}
		if ( BscMeasureDataFrequency.FREQUENCY_QUARTER.equals(frequency) 
			...
			if ( StringUtils.isBlank( startYearDate ) || StringUtils.isBlank( endYearDate ) ) {
				super.throwMessage("startYearDate|endYearDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg2"));			
			}
			if ( !StringUtils.isBlank( startYearDate ) || !StringUtils.isBlank( endYearDate ) ) {
				if ( !SimpleUtils.isDate( startYearDate+"/01/01" ) ) {
					super.throwMessage("startYearDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg6"));		
				}
				if ( !SimpleUtils.isDate( endYearDate+"/01/01" ) ) {
					super.throwMessage("endYearDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg7"));					
				}
				if ( Integer.parseInt( endYearDate.replaceAll("/", "").replaceAll("-", "") )
						< Integer.parseInt( startYearDate.replaceAll("/", "").replaceAll("-", "") ) ) {
					super.throwMessage("startYearDate|endYearDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg8"));	
				}					
			}			
		}		
		...
		if ("organization".equals(dataFor) 
			...
			super.throwMessage("measureDataOrganizationOid", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg9"));
		}
		if ("employee".equals(dataFor)
				&& this.isNoSelectId(this.getFields().get("measureDataEmployeeOid")) ) {
			super.throwMessage("measureDataEmployeeOid", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg10"));
		}
	}		
	...
	private void checkDateRange() throws ControllerException, Exception {
		...
		if (BscMeasureDataFrequency.FREQUENCY_DAY.equals(frequency) 
			...
			if ( betweenMonths >= 12 ) {
				super.throwMessage("startDate|endDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg11"));
			}
			return;
		}
		...
		if (BscMeasureDataFrequency.FREQUENCY_QUARTER.equals(frequency)) {
			if ( betweenYears >= 3 ) {
				super.throwMessage("startYearDate|endYearDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg12"));			
			}
		}
		if (BscMeasureDataFrequency.FREQUENCY_HALF_OF_YEAR.equals(frequency)) {
			if ( betweenYears >= 4 ) {
				super.throwMessage("startYearDate|endYearDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg13"));		
			}			
		}
		if (BscMeasureDataFrequency.FREQUENCY_YEAR.equals(frequency)) {
			if ( betweenYears >= 6 ) {
				super.throwMessage("startYearDate|endYearDate", this.getText("MESSAGE.BSC_PROG003D0005Q_contentQuery_msg14"));			
			}			
		}
	}
	...
	/**
	 * bsc.objectivesDashboardContentAction.action
	 */
	@ControllerMethodAuthority(programId="BSC_PROG003D0005Q")
